...
$$
而由题意，$i=0$，结合上式可知，当$n>1$时，有$c_n^{(1)}(t)=0$，从而$P_{n \leftarrow 0}^{(1)}(t)=0$；当$n=1$时，此时
$$
c_n^{(1)}(t)=-\frac{\mathrm{i} a_0}{\hbar} \sqrt{\frac{\hbar}{2 \mu \omega_0}} \frac{\mathrm{e}^{(\mathrm{i} \omega_0-\frac{1}{\tau}) t}-1}{\mathrm{i} \omega_0-\frac{1}{\tau}}
$$
有$P_{1 \leftarrow 0}^{(1)}(t)=?$，从而$t$足够大时，可得
$$
\lim_{t \rightarrow \infty} P_{1 \leftarrow 0}^{(1)}(t)=?
$$
